    ...
    <div className="min-h-screen overflow-x-hidden bg-background text-foreground">
      <header className="fixed inset-x-0 top-0 z-50 border-b border-border bg-background/85 backdrop-blur-xl">
        <div className="mx-auto flex h-20 max-w-[1440px] items-center justify-between px-5 md:px-10">
          ...
          <div className="ml-auto flex items-center gap-1 lg:ml-0 lg:gap-4"><nav className={`hidden items-center lg:flex ${language === "fr" ? "gap-5 xl:gap-6" : "gap-7"}`} aria-label="Main navigation">{nav.map(([label, id]) => <a key={id} href={`#${id}`} className={`whitespace-nowrap uppercase text-muted-foreground transition-colors hover:text-foreground ${language === "fr" ? "text-[10px] tracking-[0.1em] xl:text-[11px]" : "text-xs tracking-[0.12em]"}`}>{label}</a>)}</nav><LanguageMenu language={language} onChange={setLanguage}/><Button variant="ghost" size="icon" className="lg:hidden" onClick={() => setMenuOpen(!menuOpen)} aria-label="Toggle navigation">{menuOpen ? <X /> : <Menu />}</Button></div>
        </div>
